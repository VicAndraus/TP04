	...
	do {
		...
		switch(flag) {
			case 0:
				...
				break;
			case 1:
				cout << "Nome do funcionário?" << endl << endl;
				cin >> nometemp;
				arrayF[counter].setNome(nometemp);
				
				do {
					prontFlag = 0;
					cout << "Prontuário do funcionário?" << endl << endl;
					cin >> prontuariotemp;
					...
					for (int i = 0; i < 100; i++) {
							if (prontuariotemp == arrayF[i].getProntuario()) {
								cout << "Esse prontuário já está sendo utilizado, utilize outro." << endl;
								prontFlag = 0;
								break;
							}
							else {
			...
			case 2:
				...
				cout << "Digite o n° do prontuário que deseja excluir." << endl;
				cin >> procPront;
			...
			case 4:
				...
				for (int cont = 0; cont < 100; cont++) {
					...
					{
						cout << "Não há funcionários para exibir" << endl;
					}
				}
